...
def setup_tensorrt_engines(policy, trt_engine_path, mode=InferenceMode.n17_full_pipeline):
    ...
    if mode == InferenceMode.a2a_action_head:
        if getattr(policy.model.config, "model_type", None) != "Gr00tN1d7A2A":
            raise ValueError(
                "a2a_action_head inference mode requires a Gr00tN1d7A2A policy"
            )
        # The A2A engine is a single fused, fixed-step action head and has its
        # own metadata/SHA contract. Route it before the original N1.7
        # multi-engine bundle checks below.
        return _setup_a2a_action_head(policy, trt_engine_path)
...
def _setup_a2a_action_head(policy, trt_engine_path):
    """Install the fused A2A head without importing TensorRT until selected."""
    try:
        from scripts.deployment.a2a_trt_model_forward import setup_a2a_tensorrt_engine
    except ModuleNotFoundError:
        # Direct execution places scripts/deployment, rather than the repository
        # root, on sys.path.
        from a2a_trt_model_forward import setup_a2a_tensorrt_engine

    return setup_a2a_tensorrt_engine(policy, trt_engine_path)
...
_INFERENCE_MODE_DISPATCH = {
    InferenceMode.a2a_action_head: _setup_a2a_action_head,
