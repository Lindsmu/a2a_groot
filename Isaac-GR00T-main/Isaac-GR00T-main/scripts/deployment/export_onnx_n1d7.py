...
def prepare_observation(policy, dataset, traj_idx=0):
    ...
    if getattr(policy.model.config, "model_type", None) == "Gr00tN1d7A2A":
        bounds = compute_a2a_window_bounds(len(traj), modality_configs)
        if bounds.num_steps <= 0:
            raise ValueError(
                f"Episode {traj_idx} has no complete A2A history/future window"
            )
        data_point = extract_a2a_step_data(
            traj,
            bounds.valid_start,
