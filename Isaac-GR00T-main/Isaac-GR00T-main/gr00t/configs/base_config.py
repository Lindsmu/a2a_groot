...
from .model import MODEL_CONFIG_TYPES, create_model_union_type
    ...
    def load_dict(self, data: dict):
        if "model" in data:
            ...
            if model_type is None:
                ...
                model_class = MODEL_CONFIG_TYPES.get(model_type)
                if model_class is None:
                    raise ValueError(
                        f"Unknown model_type {model_type!r}; available types are "
                        f"{sorted(MODEL_CONFIG_TYPES)}"
                    )
            self.model = model_class(**model_data)
        if "data" in data:
