...
class A2AChannelSpec:
    """Map one controller action group into the canonical A2A trajectory space.

    ``continuous`` groups are encoded by the shared action autoencoder. ``regression``
    and ``binary`` groups are predicted by the auxiliary head. ``categorical`` is
    reserved for explicitly configured one-hot groups; scalar token channels should
    remain ``unsupported`` until a class vocabulary is supplied.
    """

    action_key: str
    source_state_key: str | None = None
    kind: Literal[
        "auto", "continuous", "regression", "binary", "categorical", "unsupported"
    ] = "auto"
    canonical_format: Literal[
        "auto", "joint_position", "xyz_rot6d", "scalar", "token"
    ] = "auto"
