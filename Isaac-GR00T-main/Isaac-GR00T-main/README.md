...
For the optional 512-D latent Action-to-Action Flow Matching model variant,
including its strict canonical data contract and the reason raw LIBERO
state/action columns cannot be used directly, see
[`getting_started/latent_a2a.md`](getting_started/latent_a2a.md). The complete
file-by-file implementation and operating reference is available in
[`getting_started/潜空间A2A完整实现与使用手册.md`](getting_started/潜空间A2A完整实现与使用手册.md).
The optional temporal-token DiT comparison backend is documented in
[`getting_started/潜空间A2A_DiT对比实现与使用.md`](getting_started/潜空间A2A_DiT对比实现与使用.md).
