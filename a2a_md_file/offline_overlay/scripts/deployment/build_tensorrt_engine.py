...
from gr00t.deployment.a2a_artifacts import sha256_file
    ...
    for inp in model.graph.input:
        ...
        for i, d in enumerate(dims):
            if d.dim_value > 0:
                ...
                if dim_name in {"batch", "batch_size"}:
...
def build_a2a_action_head_engine(
    onnx_path: str,
    engine_path: str,
    *,
    ...
    """Build the fixed-step latent-A2A head with an explicit FP32 contract.

    The A2A exporter emits a strongly-typed FP32 graph whose only dynamic
    dimensions are ``batch`` and ``vl_sequence``.  Keeping this path separate
    prevents the generic BF16 default and unknown-axis heuristics from silently
    producing an incompatible or excessively large optimization profile.
    """
    import hashlib
    import json
    from pathlib import Path

    if vl_sequence_length <= 0:
        raise ValueError("vl_sequence_length must be positive")
    if max_batch <= 0:
        raise ValueError("max_batch must be positive")

    onnx_file = Path(onnx_path)
    engine_file = Path(engine_path)
    if engine_file.name != "a2a_action_head.engine":
        raise ValueError(
            "a2a_action_head mode requires engine filename a2a_action_head.engine"
        )
    metadata_source = onnx_file.parent / "a2a_export_metadata.json"
    if not metadata_source.is_file():
        raise FileNotFoundError(
            f"Missing {metadata_source}; export the A2A action head and metadata together"
        )
    with metadata_source.open("r", encoding="utf-8") as handle:
        metadata = json.load(handle)
    if metadata.get("model_type") != "Gr00tN1d7A2A":
        raise ValueError(
            "a2a_export_metadata.json does not describe a Gr00tN1d7A2A export"
        )
    for key in (
        "onnx_sha256",
        "checkpoint_sha256",
        ...
        if not isinstance(value, str) or len(value) != 64:
            raise ValueError(f"A2A export metadata is missing a valid {key}")
    actual_onnx_sha256 = sha256_file(onnx_file)
    if metadata["onnx_sha256"] != actual_onnx_sha256:
        raise ValueError(
            "A2A ONNX SHA-256 mismatch: "
            f"expected={metadata['onnx_sha256']}, actual={actual_onnx_sha256}"
        )
    if metadata.get("verification") != "onnx_checker+torch_ort_rtol1e-4_atol1e-4":
        raise ValueError(
            "Refusing to build an A2A TensorRT engine from an ONNX graph that was not "
            "validated with ONNX checker and the PyTorch/ORT numerical oracle"
        )

    min_shapes, opt_shapes, max_shapes = derive_shapes_with_hint(
        str(onnx_file),
        opt_seq_lens={"vl_sequence": int(vl_sequence_length)},
    ...
    with engine_file.open("rb") as handle:
    ...
    metadata.update(
        {
            ...
            "build_profiles": {
                "min": {name: list(shape) for name, shape in min_shapes.items()},
                "opt": {name: list(shape) for name, shape in opt_shapes.items()},
                "max": {name: list(shape) for name, shape in max_shapes.items()},
            },
        }
    )
    engine_file.parent.mkdir(parents=True, exist_ok=True)
    metadata_destination = engine_file.parent / "a2a_export_metadata.json"
...
class BuildConfig:
    ...
    """Build mode: generic single, latent-A2A head, or the full N1.7 pipeline."""
    ...
    a2a_vl_sequence_length: int = 512
    """A2A action-head optimal VLM sequence length."""

    a2a_max_batch: int = 1
    """A2A action-head maximum dynamic batch size."""
...
def main(args: BuildConfig | None = None, trt_severity=None):
    ...
    elif args.mode == "a2a_action_head":
        if not args.onnx or not args.engine:
            raise ValueError(
                "--onnx and --engine are required in a2a_action_head mode"
            )
        build_a2a_action_head_engine(
            onnx_path=args.onnx,
            engine_path=args.engine,
            vl_sequence_length=args.a2a_vl_sequence_length,
            max_batch=args.a2a_max_batch,
            workspace_mb=args.workspace,
            trt_severity=trt_severity,
        )
    else:
