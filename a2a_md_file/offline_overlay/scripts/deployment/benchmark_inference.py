...
def main(args: BenchmarkConfig | None = None):
    ...
    is_a2a = getattr(policy.model.config, "model_type", None) == "Gr00tN1d7A2A"
    a2a_bounds = (
        compute_a2a_window_bounds(len(episode_data), modality_config) if is_a2a else None
    )
    if a2a_bounds is not None and a2a_bounds.num_steps <= 0:
        raise ValueError("Benchmark episode has no complete A2A temporal window")

    def extract_benchmark_step(step_idx):
        if is_a2a:
            return extract_a2a_step_data(
                episode_data,
                step_index=step_idx,
                modality_configs=modality_config,
                embodiment_tag=EmbodimentTag.resolve(args.embodiment_tag),
            )
        return extract_step_data(
            episode_data,
            step_index=step_idx,
            modality_configs=modality_config,
            embodiment_tag=EmbodimentTag.resolve(args.embodiment_tag),
    ...
    if args.use_trajectory:
        ...
        observations = []
        step_indices = (
            range(a2a_bounds.valid_start, a2a_bounds.valid_end + 1)
            if a2a_bounds is not None
            else range(trajectory_length)
        )
        for step_idx in step_indices:
            try:
                step_data = extract_benchmark_step(step_idx)
                obs = {
        ...
        step_index = a2a_bounds.valid_start if a2a_bounds is not None else 0
        step_data = extract_benchmark_step(step_index)
    ...
    if args.trt_engine_path and os.path.exists(args.trt_engine_path):
        ...
        if args.trt_mode in ("a2a_action_head", "n17_full_pipeline", "vit_llm_only"):
