...
def test_load_dict_restores_registered_a2a_model_subclass():
    """Selecting A2A at the standard finetune entry must not retain N1.7 defaults."""
    Config, _ = _import_config()
    from gr00t.configs.model.gr00t_n1d7_a2a import Gr00tN1d7A2AConfig

    loaded = Config().load_dict({"model": {"model_type": "Gr00tN1d7A2A"}})

    assert isinstance(loaded.model, Gr00tN1d7A2AConfig)
    assert loaded.model.action_horizon == 8
    assert loaded.model.a2a_latent_dim == 512


def test_load_dict_preserves_temporal_dit_architecture_fields():
    """DiT experiments must survive the same registered config loading path."""
    Config, _ = _import_config()

    loaded = Config().load_dict(
        {
            "model": {
                "model_type": "Gr00tN1d7A2A",
                "a2a_flow_backbone": "dit",
                "a2a_dit_token_dim": 64,
                "a2a_dit_num_layers": 6,
                "a2a_dit_num_heads": 8,
    ...
    assert loaded.model.a2a_flow_backbone == "dit"
    assert loaded.model.a2a_dit_token_dim == 64
    assert loaded.model.a2a_dit_num_layers == 6
    assert loaded.model.a2a_dit_num_heads == 8
    assert loaded.model.a2a_architecture["latent_layout"] == "8_temporal_tokens_x_64"
